...
export const register = async (req: Request, res: Response) => {
  try {
    ...
    const [newUser] = await pool.query(
      'SELECT uuid, email, username, created_at FROM users WHERE id = ?',
...
export const login = async (req: Request, res: Response) => {
  try {
    ...
    const [users] = await pool.query(
      'SELECT id, uuid, email, username, password_hash FROM users WHERE email = ?',
      [email]
    );
    ...
    await pool.query(
      ...
      [user.id, sessionToken, expiresAt, req.headers['user-agent'] || '', req.ip || '']
    );
