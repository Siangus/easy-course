...
export const proxyCoursePage = async (req, res) => {
    try {
       ...
       WHERE c.uuid = ?`, [courseId]);
        ...
        // 解密凭证
        const decrypted = decryptCredentials(course.encrypted_credentials, course.iv, course.auth_tag);
...
export const proxyVideo = async (req, res) => {
    try {
        ...
        // 获取课程凭证
        const [courses] = await pool.query('SELECT * FROM courses WHERE uuid = ?', [courseId]);
        if (!Array.isArray(courses) || courses.length === 0) {
...
// 辅助函数：获取认证cookie
async function getAuthCookies(course, credentials) {
    // 这里应该实现获取目标网站认证cookie的逻辑
    // 这可能涉及模拟登录、会话管理等
    // 返回格式：'session_id=abc123; token=xyz789'
    return '';
}
export const proxyRedirect = async (req, res) => {
    try {
