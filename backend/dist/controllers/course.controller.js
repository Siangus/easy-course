...
export const createCourse = async (req, res) => {
    try {
        ...
        const [users] = await pool.query('SELECT id FROM users WHERE uuid = ?', [userId]);
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [
        ...
        const [newCourse] = await pool.query('SELECT uuid as id, course_name as courseName, course_url as courseUrl, description, created_at FROM courses WHERE id = ?', [insertResult.insertId]);
...
export const getCourses = async (req, res) => {
    try {
        ...
        // 获取用户ID
        console.log('查询用户信息，uuid:', userId);
        const [users] = await pool.execute('SELECT id FROM users WHERE uuid = ?', [userId]);
        console.log('用户查询结果:', users);
        if (!Array.isArray(users) || users.length === 0) {
            console.error('用户不存在');
            return res.status(404).json({ error: '用户不存在' });
        ...
        const [countResult] = await pool.query('SELECT COUNT(*) as total FROM courses WHERE user_id = ? AND is_active = TRUE', [user.id]);
        console.log('课程总数查询结果:', countResult);
        const total = countResult[0].total;
        // 获取课程列表
        console.log('查询课程列表，用户ID:', user.id, 'limit:', limit, 'offset:', offset);
        const [courses] = await pool.query(`SELECT uuid as id, course_name as courseName, course_url as courseUrl, login_url as loginUrl, description, 
              last_accessed, access_count, created_at 
       FROM courses 
       WHERE user_id = ? AND is_active = TRUE 
       ORDER BY created_at DESC 
       LIMIT ? OFFSET ?`, [user.id, parseInt(limit.toString()), parseInt(offset.toString())]);
...
export const getCourseById = async (req, res) => {
    try {
        ...
        const [courses] = await pool.query(`SELECT uuid as id, course_name as courseName, course_url as courseUrl, login_url as loginUrl, description, 
              last_accessed, access_count, created_at 
       FROM courses 
       WHERE uuid = ? AND user_id = ? AND is_active = TRUE`, [courseId, user.id]);
...
export const updateCourse = async (req, res) => {
    try {
        ...
        const user = users[0];
        // 检查课程是否存在并属于该用户
        const [courses] = await pool.query('SELECT id FROM courses WHERE uuid = ? AND user_id = ?', [courseId, user.id]);
        if (!Array.isArray(courses) || courses.length === 0) {
        ...
        if (username && password) {
         ...
         WHERE id = ?`, [courseName, description || null, loginUrl || null, course.id]);
        }
        // 获取更新后的课程
        const [updatedCourse] = await pool.query('SELECT uuid as id, course_name as courseName, course_url as courseUrl, login_url, description, created_at FROM courses WHERE id = ?', [course.id]);
        res.json({
            success: true,
            message: '课程更新成功',
...
export const deleteCourse = async (req, res) => {
    try {
        ...
        const course = courses[0];
        // 软删除课程
        await pool.query('UPDATE courses SET is_active = FALSE WHERE id = ?', [course.id]);
        res.json({
            success: true,
            message: '课程删除成功'
        });
...
export const launchCourse = async (req, res) => {
    try {
        const userId = req.user?.userId;
        const { courseId } = req.params;
        // 获取用户ID
        ...
        const user = users[0];
        // 获取课程和凭证
        const [courses] = await pool.query(`SELECT c.* 
       FROM courses c 
       JOIN users u ON c.user_id = u.id 
       WHERE c.uuid = ? AND c.user_id = ?`, [courseId, user.id]);
        if (!Array.isArray(courses) || courses.length === 0) {
            return res.status(404).json({ error: '课程不存在' });
        }
        const course = courses[0];
        // 更新访问记录
        await pool.query('UPDATE courses SET last_accessed = NOW(), access_count = access_count + 1 WHERE id = ?', [course.id]);
        // 记录访问日志
        await pool.query(`INSERT INTO access_logs (user_id, course_id, access_type, success) 
       VALUES (?, ?, 'direct', TRUE)`, [user.id, course.id]);
        // 生成一次性访问令牌
